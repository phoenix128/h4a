...
import {
    ...
    removeItem,
    setProperty,
} from '@h4a/dnd-kit-sortable-tree/utilities';
import type {
    FlattenedItem,
    ItemChangedReason,
    SensorContext,
...
import { SortableTreeItem } from '@h4a/dnd-kit-sortable-tree/SortableTreeItem';
...
>({
    ...
    const handleCollapse = useCallback(
        function handleCollapse(id: string) {
            const item = findItemDeep(itemsRef.current, id)!;
            onItemsChanged(
                setProperty(itemsRef.current, id, 'collapsed', ((
                    value: boolean
                ) => {
                    return !value;
                }) as any),
    ...
    return (
        ...
        >
            <SortableContext
                items={sortedIds}
                strategy={
                    disableSorting ? undefined : verticalListSortingStrategy
                }
            >
                {flattenedItems.map((item) => {
                    return (
                        <SortableTreeItem
                            ...
                            indicator={indicator}
                            collapsed={Boolean(
                                item.collapsed && item.children?.length
                            )}
                            onCollapse={
                ...
                {isBrowser &&
                    createPortal(
                        <DragOverlay
                            dropAnimation={
                                dropAnimation ?? dropAnimationDefaultConfig
                            }
                        >
                            {activeId && activeItem ? (
                                <TreeItemComponent
    ...
    function handleDragEnd({ active, over }: DragEndEvent) {
        ...
        if (projected && over) {
            ...
            const clonedItems: FlattenedItem<TreeItemData>[] =
                flattenTree(items);
        ...
        if (overId && projected) {
            ...
            if (!previousItem) {
                ...
                if (projected.depth > previousItem.depth) {
                    ...
                    let previousSibling:
                        | FlattenedItem<TreeItemData>
                        | undefined = previousItem;
                    while (
                        previousSibling &&
